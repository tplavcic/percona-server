...
if [ $ENABLE = 1 -o $DISABLE = 1 ]; then
  ...
  if [ $? -ne 0 ]; then
    printf "ERROR: Failed to check TokuDB engine plugin status! Please check username, password and other options...\n";
    exit 1
  fi
...
# Check SELinux status - needed for TokuBackup
if [ $ENABLE_TOKUBACKUP = 1 -a -n "$(which sestatus)" ]; then
  ...
  if [ $STATUS_SELINUX = "enabled" ]; then
    ...
    if [ $MODE_SELINUX = "enforcing"  ]; then
      printf "ERROR: SELinux is in enforcing mode and needs to be disabled (or put into permissive mode) for TokuBackup to work correctly.\n\n"
      exit 1
    else
